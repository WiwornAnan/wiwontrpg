...
repo: `WiwornAnan/wiwontrpg` — โค้ดอยู่บน branch `main` แล้ว (push ให้เรียบร้อย)

```bash
# อ้างอิง (ถ้าต้อง push เอง)
git remote add origin https://github.com/WiwornAnan/wiwontrpg.git
...
2. มันจะให้สร้าง project อัตโนมัติ — ตั้งชื่ออะไรก็ได้ เช่น `wiwontrpg`, region เลือก **Singapore** (ใกล้ไทย)
3. กด **Create project**
...
3. กด **Connect** repo `wiwontrpg`
   - ถ้าไม่เห็น repo → **Configure account** → อนุญาต repo `wiwontrpg` → กลับมา
4. เลือก **Branch = `main`**
5. Render อ่าน `render.yaml` เอง แล้วโชว์เซอร์วิส `wiwontrpg`
6. กด **Apply**
...
Render จะถามค่าที่ต้องกรอกเอง — ไปที่เซอร์วิส `wiwontrpg` → เมนูซ้าย **Environment**:
...
2. กดลิงก์ที่ Render ให้ (เช่น `https://wiwontrpg.onrender.com`)
...
1. Render → `wiwontrpg` → **Settings → Custom Domains** → เพิ่มโดเมน
2. Render ให้ค่า DNS มา (มักเป็น CNAME → `wiwontrpg.onrender.com`)
3. แผง DNS ของ Hostinglotus → เพิ่มเรคคอร์ดตามที่ Render บอก
   - `www` → **CNAME** → `wiwontrpg.onrender.com`
