...
var state={egoFaces:(window.__wiwonEgoFaces||20),egoMode:"normal",ambientMode:"normal",fortuityMode:"normal",egoTurn:0,ambientTurn:0,fortuityTurn:0,topOrbit:0,topSpin:0,bottomOrbit:0,bottomSpin:0};
...
// --- Shuffled face layout ---------------------------------------------------
// A real die never carries its numbers in 1,2,3… order around any axis, so each
// ring gets a shuffled permutation: PERMS[ring][positionIndex] = value shown at
// that slot. The pivots/markers/snapping all resolve a value's slot through
// posOf(), so outcomes stay perfectly random and the indicators still land on
// the drawn numeral — only the *arrangement* changes to feel like a thrown die.
function makePerm(n){var a=[];for(var i=0;i<n;i++)a.push(i+1);for(var i=n-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var t=a[i];a[i]=a[j];a[j]=t;}return a;}
var PERMS={ambient:makePerm(8),fortuity:makePerm(10),ego:makePerm(20)};
function posOf(ring,v){return PERMS[ring].indexOf(v);}
function renderRingNumerals(groupId,R,count,dir,fontSize,color,perm){var g=document.getElementById(groupId);if(!g)return;var html="";for(var i=0;i<count;i++){var deg=dir*(360/count)*i;var rad=(deg*Math.PI/180)-Math.PI/2;var x=200+R*Math.cos(rad);var y=200+R*Math.sin(rad);html+='<text x="'+x.toFixed(1)+'" y="'+y.toFixed(1)+'" text-anchor="middle" dominant-baseline="central" font-size="'+fontSize+'" fill="'+color+'" font-family="var(--font-sans)" font-weight="500" transform="rotate('+deg.toFixed(1)+' '+x.toFixed(1)+' '+y.toFixed(1)+')">'+romanize(perm[i])+'</text>';}g.innerHTML=html;}
function renderEgoRing(faces){var count=faces;var fontSize=count<=6?23:count<=10?19:count<=14?16:13;PERMS.ego=makePerm(count);renderRingNumerals("ego-numerals",158,count,-1,fontSize,"#f7dca0",PERMS.ego);}
...
function ringLabel(r){return r==='ego'?"(d"+state.egoFaces+")":r==='ambient'?"(d8)":"(d10)";}
...
function snapRing(rawDeg,count,dir,ring){var step=360/count;var idx=Math.round(rawDeg/(dir*step));var snappedDeg=dir*step*idx;var slot=((idx%count)+count)%count;var value=PERMS[ring][slot];return{snappedDeg:snappedDeg,value:value};}
dragZone.addEventListener("pointerdown",function(evt){dragging=true;dragRing=ringAt(evt);dragZone.setPointerCapture(evt.pointerId);dragZone.style.cursor="grabbing";lastAngle=angleFromEvent(evt);dragStart=state[dragRing+"Turn"];resetEffects();clearMarkers(dragRing);PIVOTS[dragRing].style.transition="none";});
dragZone.addEventListener("pointermove",function(evt){if(!dragging)return;var ang=angleFromEvent(evt);var diff=ang-lastAngle;if(diff>180)diff-=360;if(diff<-180)diff+=360;lastAngle=ang;dragStart+=diff;state[dragRing+"Turn"]=dragStart;PIVOTS[dragRing].style.transform="rotate("+dragStart+"deg)";});
function endDrag(){if(!dragging)return;dragging=false;dragZone.style.cursor="grab";var d=dragRing;PIVOTS[d].style.transition="transform .5s cubic-bezier(.17,.67,.2,1)";var snap=snapRing(state[d+"Turn"],ringCount(d),DIRS[d],d);state[d+"Turn"]=snap.snappedDeg;PIVOTS[d].style.transform="rotate("+state[d+"Turn"]+"deg)";document.getElementById(d+"-result").textContent="ตั้งค่าด้วยมือ: "+snap.value+" "+ringLabel(d);showRollMarkers(d,ringCount(d),DIRS[d],"normal",null,null,snap.value);var vEgo=snapRing(state.egoTurn,state.egoFaces,-1,'ego').value,vAmb=snapRing(state.ambientTurn,8,1,'ambient').value,vFor=snapRing(state.fortuityTurn,10,-1,'fortuity').value;checkSpecialEffects([{value:vEgo},{value:vAmb},{value:vFor}]);emitRoll(vEgo,vAmb,vFor,true);}
...
export function DiceRoller({ open, onClose, egoFaces = 20 }: { open: boolean; onClose: () => void; egoFaces?: number }) {
  ...
  useEffect(() => {
    ...
    if (!host) return;
    (window as unknown as { __wiwonEgoFaces?: number }).__wiwonEgoFaces = egoFaces;
    ...
    // Drop the wheel in; it waits for the player to press roll (no auto-spin).
