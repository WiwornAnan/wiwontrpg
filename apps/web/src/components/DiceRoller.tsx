...
function endDrag(){if(!dragging)return;dragging=false;dragZone.style.cursor="grab";var d=dragRing;PIVOTS[d].style.transition="transform .5s cubic-bezier(.17,.67,.2,1)";var snap=snapRing(state[d+"Turn"],ringCount(d),DIRS[d]);state[d+"Turn"]=snap.snappedDeg;PIVOTS[d].style.transform="rotate("+state[d+"Turn"]+"deg)";document.getElementById(d+"-result").textContent="ตั้งค่าด้วยมือ: "+snap.value+" "+ringLabel(d);showRollMarkers(d,ringCount(d),DIRS[d],"normal",null,null,snap.value);var vEgo=snapRing(state.egoTurn,state.egoFaces,-1).value,vAmb=snapRing(state.ambientTurn,8,1).value,vFor=snapRing(state.fortuityTurn,10,-1).value;checkSpecialEffects([{value:vEgo},{value:vAmb},{value:vFor}]);emitRoll(vEgo,vAmb,vFor,true);}
