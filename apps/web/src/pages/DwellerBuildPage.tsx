                      ...
                      {featRows.map((r) => {
                        ...
                        return (
                          <div key={r.key} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 10px', border: '1px solid #ece9e3', borderRadius: 9, background: '#fff', flexWrap: 'wrap' }}>
                            ...
                            {campaignId && <button onClick={() => logRef.current('feature', `✦ ${r.name}${t.cp ? ` · CP ${t.cp}` : ''}`, r.item ? { itemId: r.item.id, isFeature: true } : undefined)} title="ส่งเข้า Log ของแคมเปญ" style={{ flex: 'none', border: '1px solid #cbb8ec', background: '#f6f2fd', color: '#5b3fa0', borderRadius: 7, padding: '4px 9px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>📋 Log</button>}
                            {active && (
                              <>
