                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {rows.map((r) => (
                          <div key={r.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 11px', border: '1px solid #e2d7f4', borderRadius: 9, background: '#faf8fd' }}>
                            ...
                            <label title="KP ของเวทนี้" style={{ flex: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 800, color: '#7a6aa0' }}>
                              KP
                      ...
                      {featRows.map((r) => {
                        ...
                        return (
                          <div key={r.key} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 10px', border: '1px solid #ece9e3', borderRadius: 9, background: '#fff', flexWrap: 'wrap' }}>
                            ...
                            {campaignId && <button onClick={() => logRef.current('feature', `✦ ${r.name}${t.cp ? ` · KP ${t.cp}` : ''}`, r.item ? { itemId: r.item.id, isFeature: true } : undefined)} title="ส่งเข้า Log ของแคมเปญ" style={{ flex: 'none', border: '1px solid #cbb8ec', background: '#f6f2fd', color: '#5b3fa0', borderRadius: 7, padding: '4px 9px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>📋 Log</button>}
                            {active && (
                              <>
                            ...
                            <label style={{ flex: 'none', display: 'flex', alignItems: 'center', gap: 3, fontSize: 9, color: '#5b3fa0', fontWeight: 600 }}>KP<input defaultValue={t.cp ?? ''} key={`cp${t.cp}`} onBlur={(e) => setFeatTrack(r.key, { cp: Math.max(0, Math.round(Number(e.target.value) || 0)) })} inputMode="numeric" placeholder="0" style={{ width: 38, border: '1px solid #d6c7f0', borderRadius: 6, textAlign: 'center', fontSize: 11, padding: '3px 2px', outline: 'none', background: '#faf8fd' }} /></label>
